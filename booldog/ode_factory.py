...
def ode_factory(network, transform, **kwargs):
    ...
    Parameters
    ----------
    network : :py:class:`RegulatoryNetwork` or :py:class:`BooleanNetwork`
        Input network.
...
class ODE(object):
    ...
    def __init__(self, baseclass, network, transform, **kwargs):
        '''Initialise ODE
        ...
        **kwargs
        ...
        super(self.__class__, self).__init__(transform, **kwargs)


        parent_class_name = self.__class__.__mro__[1].__name__
        logger.info(
            "Creating ODE system for %s using class %s", transform, parent_class_name
        )
...
class BooleCubeODE(object):
    ...
    def homologue_b1(self):
        ...
        all_B1s = ['0'] * self.boolean_network.n
        for node in self.boolean_network.nodes:  # iterate over all nodes
            parents = self.boolean_network.get_parents(node)
...
class SquadODE(object):
    ...
    def __init__(self, transform, gamma=1, h=10, **kwargs):
        ...
        self.param_gamma = parameter_to_array(gamma, self.boolean_network.index)
        self.param_h = parameter_to_array(h, self.boolean_network.index)

        print(self.param_gamma)
        print(self.param_h)
        # matrices
        self.Act, self.Inh = self.boolean_network.primes_to_matrices()
...
transforms = set(ode_parent_classes)
